...
class _OvertimeDetailScreenState extends State<OvertimeDetailScreen> {
  ...
  Future<void> _openAttachment(String url) async {
    ...
    try {
      ...
      final savePath = '${dir.path}/overtime_attachments/$fileName';
      ...
      await Directory('${dir.path}/overtime_attachments').create(recursive: true);
  ...
  Widget build(BuildContext context) {
    ...
    final dateLabel = overtime.isRange
        ? '${DateFormat('dd MMM yyyy').format(DateTime.parse(overtime.startDate))} - ${DateFormat('dd MMM yyyy').format(DateTime.parse(overtime.endDate))}'
        : DateFormat('dd MMMM yyyy').format(DateTime.parse(overtime.startDate));
        ...
        child: Column(
          ...
          children: [
            if (overtime.user != null) ...[
              Text(overtime.user!.username, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
            ...
            Row(
              ...
              children: [
                Expanded(
                  child: Text(overtime.title, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                ),
            ...
            _InfoRow(icon: Icons.access_time, label: 'Jam', value: '${overtime.startTime} - ${overtime.endTime}'),
            _InfoRow(icon: Icons.payments_outlined, label: 'Kompensasi', value: overtime.compensationType),
            const SizedBox(height: 12),
            ...
            if (overtime.attachmentUrls.isNotEmpty) ...[
              ...
              Container(
                ...
                child: Column(
                  children: overtime.attachmentUrls.asMap().entries.map((entry) {
                    final index = entry.key;
                    final url = entry.value;
                    final isLast = index == overtime.attachmentUrls.length - 1;
                    final fileName = url.split('/').last;
